...
## weekly_coach_messages _(Sprint 113 / #1504; daily key 2026-07)_

One persisted coaching message per `(user_id, for_date)`. Originally weekly;
now daily so post-sync / evening jobs can refresh the same day. ``for_week``
remains denormalized for display. Generated by
`backend/services/weekly_coach_message.py` (`generate_for_user`). Unique
`(user_id, for_date)` enforces idempotency. Read via
`GET /api/coach/daily-message` (and compat `weekly-message`). Migration:
`d35f3915950e_add_for_date_to_weekly_coach_messages` (adds `for_date`, drops
week-only uniqueness).
