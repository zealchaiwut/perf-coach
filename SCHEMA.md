...
The user's target race goal — one active goal per user at a time (`PUT /api/coach/goal` deactivates any prior active goal before inserting the new one). Read via `GET /api/coach/goal`. Model: `PerformanceGoal` in `backend/models.py`. Feeds the deterministic coach-plan engine (`backend/services/coach_plan.py`).
...
| target_time | int | NOT NULL — goal finish time in seconds |
| race_date | date | NOT NULL — future date (validated at the API) |
| active | bool | NOT NULL, server default `true` |
| created_at | timestamptz | server default `now()` |
...
Persisted weekly coaching message — one record per `(user, ISO week)`. Generated by `backend/services/weekly_coach_message.py` from `coach_plan.build_plan_state()` + projection outputs; the unique constraint enforces idempotency (re-running the weekly job for the same ISO week upserts the existing row rather than inserting a duplicate). Read via `GET /api/coach/weekly-message` (latest) / `GET /api/coach/weekly-messages?limit=` (history). Model: `WeeklyCoachMessage` in `backend/models.py`.
...
| text | text | NOT NULL — composed message |
| plan_state_snapshot | jsonb | nullable — engine plan-state snapshot at generation time |
