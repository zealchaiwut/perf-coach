...
(function () {
    ...
    fetch("/api/athletes/" + _athleteId + "/performance")
      ...
      .then(function (data) {
        ...
        if (state === "building_baseline") {
          var reason =
            data.reason || "Keep training: your baseline is building.";
  ...
  function _renderScoreCard(type, data) {
    ...
    var sessions = Array.isArray(data.contributing_sessions)
      ? data.contributing_sessions
      : [];
  ...
  function _renderContributingSessions(container, sessions) {
    ...
    sessions.forEach(function (s) {
      ...
      li.innerHTML =
        '<span class="perf-sess-date">' +
        _escHtml(dateStr) +
        "</span>" +
        '<span class="perf-sess-title">' +
        _escHtml(title) +
        "</span>" +
  ...
  function _loadFitnessChart(rangeKey) {
    ...
    var url =
      "/api/performance/chart?athlete_id=" +
      encodeURIComponent(_athleteId) +
      "&start_date=" +
      startDate +
      "&end_date=" +
      endDate;
          ...
          tooltip: {
            callbacks: {
              label: function (ctx) {
                var v = ctx.parsed.y;
                return (
                  ctx.dataset.label + ": " + (v != null ? v.toFixed(1) : "—")
                );
              },
            },
          },
  ...
  function _computeAcwr(series) {
    ...
    var GUIDANCE = {
      detraining:
        "Acute load is below chronic baseline. Consider gradually increasing volume to maintain fitness.",
      productive:
        "Training load is in the optimal range. Continue current stress to build fitness.",
      high_risk:
        "Acute load spike is above chronic baseline. Reduce volume to lower injury risk.",
    };
  ...
  function _renderAcwr(acwr) {
    ...
    if (bandEl) {
      ...
      bandEl.className =
        "perf-acwr-band-badge perf-acwr-band--" + (acwr.band || "");
    }
    if (guidanceEl) {
  ...
  function _renderRunPR(data) {
    ...
    var categories = [
      ...
      {
        key: "speedRecords",
        keys: ["5km", "10km", "half_marathon", "marathon", "1mile", "1km"],
      },
      ...
      .map(function (item) {
        ...
        if (rec.reason) {
          return (
            ...
            '<div class="perf-pr-name">' +
            _esc(label) +
            "</div>" +
            '<div class="perf-pr-reason">' +
            _esc(rec.reason) +
            "</div>" +
            "</div>"
          );
        ...
        var date = rec.date || "—";
        var src =
          rec.sourceWorkout && rec.sourceWorkout.id
            ? '<a class="perf-pr-link" href="/log#workout=' +
              _esc(String(rec.sourceWorkout.id)) +
              '">View workout</a>'
            : "";
        ...
        return (
          ...
          _esc(label) +
          "</div>" +
          '<div class="perf-pr-val">' +
          _esc(val) +
          "</div>" +
          '<div class="perf-pr-date">' +
          _esc(date) +
          "</div>" +
          src +
          "</div>"
