...
async def create_strength_sessions_batch(body: _StrengthBatchBody, user: User = Depends(resolve_user)):
...
async def list_strength_sessions(user: User = Depends(resolve_user)):
    sessions = _svc.list_strength_sessions(user_id=user.id)
    return JSONResponse(sessions)


@router.get("/api/strength-sessions/{session_id}")
async def get_strength_session(session_id: str, user: User = Depends(resolve_user)):
...
async def create_strength_session(body: _StrengthCreateBody, user: User = Depends(resolve_user)):
...
async def update_strength_session(session_id: str, body: _StrengthPatchBody, user: User = Depends(resolve_user)):
    sid = _parse_session_id(session_id)
...
async def delete_strength_session(session_id: str, user: User = Depends(resolve_user)):
    sid = _parse_session_id(session_id)
...
async def create_plyo_sessions_batch(body: _PlyoBatchBody, user: User = Depends(resolve_user)):
    exercises = [ex.dict() for ex in body.exercises]
...
async def list_plyo_sessions(user: User = Depends(resolve_user)):
    sessions = _svc.list_plyo_sessions(user_id=user.id)
    return JSONResponse(sessions)


@router.get("/api/plyo-sessions/{session_id}")
async def get_plyo_session(session_id: str, user: User = Depends(resolve_user)):
    sid = _parse_session_id(session_id)
...
async def create_plyo_session(body: _PlyoCreateBody, user: User = Depends(resolve_user)):
...
async def update_plyo_session(session_id: str, body: _PlyoPatchBody, user: User = Depends(resolve_user)):
    sid = _parse_session_id(session_id)
    fields = body.dict(exclude_unset=True)
...
async def delete_plyo_session(session_id: str, user: User = Depends(resolve_user)):
    sid = _parse_session_id(session_id)
