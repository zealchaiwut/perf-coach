...
class WeeklyCoachMessage(Base):
    ...
    Originally weekly (#1504); now keyed by calendar date (Asia/Bangkok) so
    post-sync / evening jobs can refresh Dream/Reflection the same day.
    ``for_week`` is kept denormalized for display. Unique (user_id, for_date)
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "for_date", name="uq_weekly_coach_messages_user_date"),
        Index("ix_weekly_coach_messages_user_generated_at", "user_id", "generated_at"),
        Index("ix_weekly_coach_messages_user_for_date", "user_id", "for_date"),
    )


class DailyBrief(Base):
    """Coach brief v4 JSON — one structured brief per (user, brief_date)."""
