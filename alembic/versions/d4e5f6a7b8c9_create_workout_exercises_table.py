...
def upgrade() -> None:
    if not table_exists("workout_exercises"):
        op.create_table(
            "workout_exercises",
            sa.Column(
                "id",
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_workout_exercises_workout_id_display_order"
        " ON workout_exercises (workout_id, display_order)"
    )


def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS ix_workout_exercises_workout_id_display_order")
