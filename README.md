...
- **Run View** — per-workout run detail page (`/run-view`) with redesigned header and load block; shows session profile, laps (with intensity bands), route strip (GPS altitude/pace sparkline), pace, power, HR, cadence, and stride-length per km split; supports both auto (1-km) and manually-entered lap rows (`lap_type`); **Compact mode** — higher information density layout collapsing per-split detail rows; **Snapshot mode** — shareable static view of a run
- **Weekly check-in** — standalone weekly check-in view at `/weekly-check-in`; `GET /api/weekly-check-in` returns a structured summary with `building` flag (True when < 7 days of data), `week_summary` (three voice-formatted lines: `weight_trend`, `habit_consistency`, `training_note`), `bright_spot` (single highlight with text and source), `next_lever` (single action with text and metric), `focus_habits` (top 3 habits with cooldown days), and `correlations_fetched` (False when building baseline); insights query is skipped entirely when building baseline; service (`backend/services/weekly_check_in.py`) is a pure function with no DB access
