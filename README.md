...
- **LLM coaching layer** _(Sprint 102)_ — `backend/services/llm.py` provides a Groq-backed LLM client (`call_llm`) with JSON-schema-constrained output, a rule-based fallback path, and a `llm_generations` DB cache (keyed by `user_id + surface + input_signature`) so repeated identical inputs never incur a second API call; fail-safe by default — `LLM_COACH_ENABLED=false` unless explicitly set. Requires `GROQ_API_KEY`; tier models configurable via `GROQ_MODEL_FAST` / `GROQ_MODEL_DEEP`. Full reference: `docs/llm-coaching.md`. Four surfaces built on top: (1) **Habit insights + nudges** (`backend/services/habit_insights.py`, `habit_nudges.py`) — LLM rephrases statistical habit-outcome insights and adherence nudges in a coaching voice; falls back to rule-based text when LLM is off or fails; (2) **Readiness explanation** (`backend/services/readiness_explanation.py`) — `get_readiness_explanation` returns a 2–3 sentence "why this score" narrative appended to `GET /api/readiness/compute` responses as an `explanation` field; (3) **Weekly summary narrative** (`backend/services/weekly_summary.py`) — `GET /api/weekly-summary` assembles load/guardrail/PR facts for the week and calls `get_narrative` which returns an LLM coach-style paragraph (or rule-based fallback); response shape: `{week_start, facts, narrative, source: "llm"|"fallback"}`; rendered as a weekly summary card on the home page; (4) **Training plan suggestions** (`backend/services/plan_suggestions.py`) — `GET /api/plan/suggestions` returns LLM-proposed next-week sessions (`{facts, suggestions: [{day_offset, workout_type, target_tss, duration_minutes, intent}], source}`); suggestions are validated against guardrail and load constraints; the Plan tab UI surfaces them with a one-tap add flow that never auto-applies sessions to the plan.
...
- **Plan races/checkpoints API** _(Sprint 92; router renamed & prefixed Sprint 98)_ — a nested REST API for managing race entries and intermediate checkpoints within a plan: `GET/POST /api/plans/{plan_id}/races`, `GET/PATCH/DELETE /api/plans/{plan_id}/races/{race_id}`, `GET/POST /api/plans/{plan_id}/races/{race_id}/checkpoints`, `PATCH/DELETE /api/plans/{plan_id}/races/{race_id}/checkpoints/{checkpoint_id}`; plan ownership is enforced (plan_id must match session user id). The router now lives in `backend/routers/projection.py` (with `backend/routers/plan.py` retained as a compatibility alias) and carries an explicit `prefix="/api"` so its routes are correctly served under `/api/` and reuse the shared `resolve_user` auth dependency (Sprint 98 / #1138, #1137, #1177)
...
| `POST /api/readiness/compute` | Computes and stores today's readiness score; response includes an `explanation` field — a 2–3 sentence LLM narrative (or rule-based fallback) explaining the score drivers |
...
| `GET /api/habits/adherence` | Returns per-habit adherence (7-day window), trend (`stable`/`declining`), best/worst day of week, and coaching nudge copy (LLM-phrased when enabled, rule-based fallback). Response: `{building (bool), reason (str\|null), habits (list)}`. `building: true` when all habits have fewer than 7 distinct logged days. HTTP 200 always. |
| `GET /api/weekly-summary` | Returns a coach-style weekly narrative + key facts. Optional `week` param (ISO date of Monday; defaults to current Bangkok week). Response: `{week_start, facts: {session_count, total_tss, total_distance_km, ctl_start, ctl_end, atl_start, atl_end, tsb_start, tsb_end, guardrail_state, guardrail_message, prs}, narrative, source: "llm"\|"fallback"}` |
| `GET /api/plan/suggestions` | Returns LLM-proposed next-week training sessions, validated against load/guardrail constraints. Response: `{facts, suggestions: [{day_offset, workout_type, target_tss, duration_minutes, intent}], source: "llm"\|"fallback"}`. Never auto-applied — one-tap add only. |
...
| `GET /api/training-load/weekly` | Weekly TSS/distance aggregates for the volume chart _(Sprint 98 / #541)_. Optional `from`/`to` params (default: last 90 days). Buckets are Monday-to-Sunday (ISO week). Returns `{weeks: [{week_start, run_tss, strength_tss, total_tss, total_distance_km}]}` — aggregates only, no per-workout serialization. |
