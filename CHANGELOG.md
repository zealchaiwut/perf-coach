...
- #658: Clearing a thresholds field in Settings no longer drifts back to a default on save — `frontend/pages/settings.html` rebuilt `_tBaseline` after a successful save by falling back to `_tDefaults` for `max_hr`, `zone2_hr_min`, `zone2_hr_max`, and `weekly_zone2_target_min` whenever the saved value was null, so a deliberately-cleared field reappeared populated. The post-save update now trusts the saved (possibly-null) value verbatim for every field
- #732: `GET /api/readiness/current` guards an empty load-curve list — when `load_curves` is empty (no training-load snapshots yet) the endpoint indexed `load_curves[-1]` and raised; it now returns `{"building_baseline": true}` in that case, matching the existing baseline-building response
- #1486: Silent gap-analysis / training-verdict failures now surface a degraded signal instead of an all-clear brief — in `backend/services/daily_brief.py::_assemble_advisories`, an exception from the gap-analysis or training-verdict source now appends an `error`-severity advisory (`gap_analysis_error` / `training_verdict_error`) rather than only logging to stderr, and `_build_brief` (both the service and `scripts/export_brief.py`) sets a new top-level `advisories_degraded` boolean when any advisory has `severity == "error"`, so a brief built from partially-failed sources is no longer indistinguishable from a genuinely all-clear one
