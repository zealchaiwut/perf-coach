"""
Tests for issue #542 — Pin Chart.js CDN to a specific version in training-log.html.
"""
import re
from pathlib import Path

TRAINING_LOG = Path(__file__).parents[1] / "frontend" / "pages" / "training-log.html"

FLOATING_TAG_RE = re.compile(
    r'cdn\.jsdelivr\.net/npm/chart\.js@(\d+(?:\.\d+)?)/dist/chart\.umd\.min\.js'
)
PINNED_VERSION_RE = re.compile(
    r'cdn\.jsdelivr\.net/npm/chart\.js@(\d+\.\d+\.\d+)/dist/chart\.umd\.min\.js'
)
...
def test_no_floating_chartjs_cdn_reference():
    """AC2: No floating major-only or minor-only tag remains in the file."""
    html = _html()
...
def test_exactly_one_chartjs_cdn_reference():
    """AC2: Only one Chart.js CDN script tag should exist in the file."""
    html = _html()
    all_matches = PINNED_VERSION_RE.findall(html)
    assert len(all_matches) == 1, (
        f"Expected exactly 1 pinned Chart.js CDN reference, found {len(all_matches)}."
    )
