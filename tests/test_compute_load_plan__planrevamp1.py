"""Unit tests for compute_load_plan (Plan tab revamp, Part 1).
...
Covers the AC items from the spec's section 1.4:
  ...
  AC4 - the ACWR ceiling clamp binds when ramp_rate is large
...
from backend.services.load_plan import compute_load_plan, TAPER_CURVE, ACWR_CEILING_MULT
...
# ── AC4: ACWR ceiling clamp ──────────────────────────────────────────────────

def test_acwr_ceiling_clamps_large_ramp():
    trailing_avg = 200.0
    ceiling = ACWR_CEILING_MULT * trailing_avg
    result = compute_load_plan(
    ...
    for w in clamped_weeks:
        assert abs(w["target_tss"] - ceiling) < 0.01
...
def test_target_never_exceeds_ceiling():
    result = compute_load_plan(
        baseline=200, ramp_rate=0.3, hold_weeks=3, taper_weeks=3, weeks_to_race=16,
        trailing_28d_avg=250,
    )
    ceiling = ACWR_CEILING_MULT * 250
    for w in result["weeks"]:
        assert w["target_tss"] <= ceiling + 0.01
