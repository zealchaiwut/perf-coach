...
def api_session_cookie(http_client, api_user_id):
    ...
    from backend.db import engine as _eng
    with _OrmSession(_eng) as db:
        u = db.get(_User, uuid.UUID(api_user_id))
        name = u.name
    res = http_client.post("/api/auth/login", json={"username": name, "password": _WE_TEST_PW})
    assert res.status_code == 200, res.text
    return res.cookies.get("session")


def _we_post(client, entry_date, weight_kg, cookie, **kw):
    payload = {"entry_date": entry_date, "weight_kg": weight_kg}
    if "entry_time" in kw:
        payload["entry_time"] = kw["entry_time"]
    if "notes" in kw:
        payload["notes"] = kw["notes"]
    return client.post(_WE_ENDPOINT, json=payload, cookies={"session": cookie})


# ── (a) POST creates entry ────────────────────────────────────────────────────

def test_api_a_post_creates_entry(http_client, api_user_id, api_session_cookie):
    """AC (a): POST returns 201 with full row including source='manual'."""
    res = _we_post(http_client, "2020-11-01", 75.0, api_session_cookie, notes="morning")
    assert res.status_code == 201, res.text
    body = res.json()
    ...
    http_client.delete(f"{_WE_ENDPOINT}/{body['id']}", cookies={"session": api_session_cookie})


# ── (b) Duplicate POST returns 409 with existing_id ──────────────────────────

def test_api_b_duplicate_post_returns_409(http_client, api_session_cookie):
    """AC (b): Duplicate (entry_date+entry_time) returns 409 with error_code and existing_id."""
    r1 = _we_post(http_client, "2020-11-02", 75.0, api_session_cookie)
    assert r1.status_code == 201, r1.text
    existing_id = r1.json()["id"]

    r2 = _we_post(http_client, "2020-11-02", 76.0, api_session_cookie)
    assert r2.status_code == 409, r2.text
    body = r2.json()
    assert body["error_code"] == "duplicate"
    assert body["existing_id"] == existing_id

    http_client.delete(f"{_WE_ENDPOINT}/{existing_id}", cookies={"session": api_session_cookie})


# ── (c) Future entry_date > 1 day returns 422 ────────────────────────────────

def test_api_c_future_entry_date_returns_422(http_client, api_session_cookie):
    """AC (c): entry_date more than 1 day in the future returns 422."""
    future = (_dt.date.today() + _dt.timedelta(days=2)).isoformat()
    res = _we_post(http_client, future, 75.0, api_session_cookie)
    assert res.status_code == 422, res.text


# ── (d) weight_kg outside 20–300 returns 422 ────────────────────────────────

def test_api_d_weight_kg_out_of_range_returns_422(http_client, api_session_cookie):
    """AC (d): weight_kg outside 20–300 returns 422."""
    for bad_kg in [5.0, 19.9, 300.1, 500.0]:
        res = _we_post(http_client, "2099-11-03", bad_kg, api_session_cookie)
...
def test_api_e_get_filters_by_date_range(http_client, api_session_cookie):
    """AC (e): GET returns only entries within the specified from/to date range."""
    r_in = _we_post(http_client, "2020-11-10", 72.0, api_session_cookie)
    r_out = _we_post(http_client, "2020-12-01", 73.0, api_session_cookie)
    ...
    http_client.delete(f"{_WE_ENDPOINT}/{id_in}", cookies={"session": api_session_cookie})
    http_client.delete(f"{_WE_ENDPOINT}/{id_out}", cookies={"session": api_session_cookie})


# ── (f) GET summary statistics are accurate ───────────────────────────────────

def test_api_f_get_summary_statistics(http_client, api_session_cookie):
    """AC (f): GET summary contains accurate aggregates."""
    data = [("2020-11-15", 70.0), ("2020-11-16", 72.0), ("2020-11-17", 68.0)]
    ids = []
    for d, w in data:
        r = _we_post(http_client, d, w, api_session_cookie)
    ...
    for eid in ids:
        http_client.delete(f"{_WE_ENDPOINT}/{eid}", cookies={"session": api_session_cookie})


# ── (g) PATCH updates weight_kg ───────────────────────────────────────────────

def test_api_g_patch_updates_weight_kg(http_client, api_session_cookie):
    """AC (g): PATCH weight_kg returns 200 with updated entry."""
    r = _we_post(http_client, "2020-11-20", 80.0, api_session_cookie)
    assert r.status_code == 201, r.text
    eid = r.json()["id"]
...
def test_api_h_patch_with_entry_date_returns_422(http_client, api_session_cookie):
    """AC (h): PATCH returns 422 when entry_date is present in the request body."""
    r = _we_post(http_client, "2020-11-21", 80.0, api_session_cookie)
    assert r.status_code == 201, r.text
    eid = r.json()["id"]
...
def test_api_i_delete_removes_entry(http_client, api_session_cookie):
    """AC (i): DELETE returns 200 with {"deleted": true} and entry is gone after."""
    r = _we_post(http_client, "2020-11-25", 75.0, api_session_cookie)
    assert r.status_code == 201, r.text
    eid = r.json()["id"]

    res = http_client.delete(f"{_WE_ENDPOINT}/{eid}", cookies={"session": api_session_cookie})
    assert res.status_code == 200, res.text
    assert res.json() == {"deleted": True}

    res2 = http_client.delete(f"{_WE_ENDPOINT}/{eid}", cookies={"session": api_session_cookie})
    assert res2.status_code == 404


# ── (j) DELETE unknown ID returns 404 ────────────────────────────────────────

def test_api_j_delete_unknown_id_returns_404(http_client, api_session_cookie):
    """AC (j): DELETE on a non-existent ID returns 404."""
    fake_id = str(uuid.uuid4())
    res = http_client.delete(f"{_WE_ENDPOINT}/{fake_id}", cookies={"session": api_session_cookie})
    assert res.status_code == 404, res.text


# ── Additional AC coverage ────────────────────────────────────────────────────

def test_api_post_anon_returns_401(http_client):
    """AC: POST without session returns 401 (replaces old user_id=unknown→404)."""
...
def test_api_get_from_after_to_returns_422(http_client, api_session_cookie):
...
def test_api_get_range_exceeds_365_returns_422(http_client, api_session_cookie):
...
def test_api_patch_unknown_id_returns_404(http_client, api_session_cookie):
...
def test_api_patch_with_user_id_returns_422(http_client, api_session_cookie):
    """AC: PATCH returns 422 when user_id is present in body."""
    r = _we_post(http_client, "2020-11-28", 77.0, api_session_cookie)
    assert r.status_code == 201, r.text
    eid = r.json()["id"]
...
def test_api_post_notes_too_long_returns_422(http_client, api_session_cookie):
