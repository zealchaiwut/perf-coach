...
def test_home_html_has_row2_for_performance():
    """AC-PERF-1: home.html must have a lower-row container for the Performance PRs card.

    #442 assembled the home layout with a JS-created Performance card inside
    `home-perf-container` (within `home-lower-row`), superseding #441's static
    `row-2` container. Accept either the new container or the legacy id.
    """
    assert (
        'id="home-perf-container"' in _HOME_HTML
        or 'id="home-lower-row"' in _HOME_HTML
        or 'id="row-2"' in _HOME_HTML
    ), (
        "home.html must have a lower-row container (home-perf-container / "
        "home-lower-row / row-2) for the Performance PRs card"
    )
...
def _workouts_container_pos() -> int:
    """Position of the Recent Workouts container, accepting #442's container id
    (`home-workouts-container`, which JS fills with the card) or #441's legacy
    static `home-workouts-card`."""
    for needle in ('id="home-workouts-container"', 'id="home-workouts-card"'):
        pos = _HOME_HTML.find(needle)
        if pos != -1:
            return pos
    return -1
...
def test_home_html_has_home_workouts_card():
    """AC-WO-1/AC-WO-5: home.html must have a Recent Workouts container.

    #442 fills `home-workouts-container` via JS; #441 used a static
    `home-workouts-card`. Accept either.
    """
    assert _workouts_container_pos() != -1, (
        "home.html must have a Recent Workouts container "
        "(home-workouts-container or home-workouts-card)"
    )


def test_home_html_workouts_card_before_sleep_card():
    """AC-WO-2: If home-sleep-card exists, the workouts container must appear before it."""
    workouts_pos = _workouts_container_pos()
    assert workouts_pos != -1, "home.html must have a Recent Workouts container"
