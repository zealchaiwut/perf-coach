...
@pytest.fixture(scope="module")
def seeded_entries(client, user_id):
    entries = [
        {"user_id": user_id, "entry_date": "2024-01-10", "weight_kg": 80.5, "entry_time": "07:00", "notes": "morning"},
        {"user_id": user_id, "entry_date": "2024-01-15", "weight_kg": 80.0, "entry_time": "08:00", "notes": "hello, world"},
        {"user_id": user_id, "entry_date": "2024-02-05", "weight_kg": 79.5},
    ]
    ids = []
    for e in entries:
        r = client.post(WE, json=e)
        assert r.status_code == 201, r.text
        ids.append(r.json()["id"])
    yield ids
    for eid in ids:
        client.delete(f"{WE}/{eid}")
...
def test_add_csv_export_endpoints__weight_entries_valid_csv_correct_headers(client, user_id, seeded_entries):
    r = client.get(EXP_WE, params={"user_id": user_id})
...
def test_add_csv_export_endpoints__weight_entries_date_range_filter(client, user_id, seeded_entries):
    r = client.get(EXP_WE, params={"user_id": user_id, "from": "2024-01-01", "to": "2024-01-31"})
    assert r.status_code == 200, r.text
    rows = _parse_csv(r.text)
    data = rows[1:]
    assert len(data) == 2
...
def test_add_csv_export_endpoints__weight_entries_empty_returns_header_only(client, user_id, seeded_entries):
    r = client.get(EXP_WE, params={"user_id": user_id, "from": "2000-01-01", "to": "2000-01-02"})
    assert r.status_code == 200, r.text
    rows = _parse_csv(r.text)
    assert rows[0] == ["entry_date", "entry_time", "weight_kg", "notes", "source"]
    assert len(rows) == 1
...
def test_add_csv_export_endpoints__weight_entries_commas_quoted(client, user_id, seeded_entries):
    r = client.get(EXP_WE, params={"user_id": user_id})
    assert r.status_code == 200, r.text
    rows = _parse_csv(r.text)
...
def test_add_csv_export_endpoints__weight_targets_valid_csv_achieved_pct(client, user_id):
    ...
    t_id = r_create.json().get("id")

    r = client.get(EXP_WT, params={"user_id": user_id})
    assert r.status_code == 200, r.text
    assert "text/csv" in r.headers.get("content-type", "")
    rows = _parse_csv(r.text)
    assert rows[0] == [
    ...
    if t_id:
        client.delete(f"{WT}/{t_id}")


# --- (f) Status filter returns only matching rows ---

def test_add_csv_export_endpoints__weight_targets_status_filter(client, user_id):
    r_all = client.get(EXP_WT, params={"user_id": user_id})
    ...
    r_f = client.get(EXP_WT, params={"user_id": user_id, "status": some_status})
...
def test_add_csv_export_endpoints__content_disposition_attachment(client, user_id, seeded_entries):
    r = client.get(EXP_WE, params={"user_id": user_id})
    cd = r.headers.get("content-disposition", "")
