...
def test_ac2_zone2_detection_references_prefs_min():
    """renderRunView must use the user preference zone2_hr_min when available."""
    src = _src()
    assert "zone2_hr_min" in src, (
        "training-log.js must reference zone2_hr_min from user preferences "
        "for per-lap Zone 2 detection"
    )


def test_ac2_zone2_detection_references_prefs_max():
    """renderRunView must use the user preference zone2_hr_max when available."""
    src = _src()
    assert "zone2_hr_max" in src, (
        "training-log.js must reference zone2_hr_max from user preferences "
        "for per-lap Zone 2 detection"
    )


def test_ac2_zone2_detection_falls_back_to_constants():
    """The fallback to ZONE2_HR_MIN / ZONE2_HR_MAX must still be present."""
    src = _src()
    assert "ZONE2_HR_MIN" in src, (
        "training-log.js must keep ZONE2_HR_MIN as the fallback constant"
    )
    assert "ZONE2_HR_MAX" in src, (
        "training-log.js must keep ZONE2_HR_MAX as the fallback constant"
    )


def test_ac2_zone2_detection_uses_effective_values_in_split_loop():
    """The split-level _zone2 flag must be computed from effective (prefs-aware)
    min/max values, not the bare constants."""
    src = _src()
    # Verify that the _zone2 assignment uses a variable (not the bare constant).
    # After the change, the code must look like:
    #   s._zone2 = (s.avg_hr != null && s.avg_hr >= z2min && s.avg_hr <= z2max)
    # or some equivalent that references a local/closure variable, not ZONE2_HR_MIN directly.
    rv_start = src.find("function renderRunView(")
    assert rv_start != -1
    brace_start = src.find("{", rv_start)
    depth = 0
    ...
    # Find the _zone2 assignment line
    z2_idx = rv_body.find("_zone2")
    assert z2_idx != -1, "renderRunView must set s._zone2 for each split"

    # Extract the line containing _zone2
    line_start = rv_body.rfind("\n", 0, z2_idx) + 1
    line_end = rv_body.find("\n", z2_idx)
    z2_line = rv_body[line_start:line_end]

    # The line must NOT directly reference ZONE2_HR_MIN / ZONE2_HR_MAX
    assert "ZONE2_HR_MIN" not in z2_line and "ZONE2_HR_MAX" not in z2_line, (
        "The s._zone2 assignment must use local effective variables (e.g. z2min/z2max), "
        "not the bare ZONE2_HR_MIN / ZONE2_HR_MAX constants"
    )
...
def test_ac3_hr_note_uses_effective_min_variable():
    ...
    # The Z2 note text should reference the effective variable, e.g. z2min
    note_idx = rv_body.find("rv-z2-note")
    assert note_idx != -1, "renderRunView must include the rv-z2-note element"

    note_end = rv_body.find("</div>", note_idx)
    note_chunk = rv_body[note_idx:note_end + 6]

    # The note must NOT hard-code ZONE2_HR_MIN / ZONE2_HR_MAX
    assert "ZONE2_HR_MIN" not in note_chunk and "ZONE2_HR_MAX" not in note_chunk, (
...
def test_ac3_hr_note_still_shows_hr_range():
    ...
    src = _src()
    # The note template should still contain 'HR' and a separator like '–' or '-'
    assert "rv-z2-note" in src, "rv-z2-note element must be present in training-log.js"
    # And must no longer say 'range will come from settings' (that was the old placeholder)
    rv_start = src.find("function renderRunView(")
    brace_start = src.find("{", rv_start)
    depth = 0
    i = brace_start
    while i < len(src):
        ...
        i += 1
    rv_body = src[rv_start:i + 1]
    assert "range will come from settings" not in rv_body, (
...
def test_ac5_null_check_before_using_prefs_value():
    """The code must guard against null zone2_hr_min/max values (using null-check or ??).
    When the prefs row has null values, ZONE2_HR_MIN / ZONE2_HR_MAX are used."""
    src = _src()
    # Look for a null-guard pattern around zone2_hr_min usage
    has_null_check = (
    ...
    assert has_null_check, (
        "training-log.js must guard zone2_hr_min for null before using it, "
        "falling back to ZONE2_HR_MIN when null/undefined"
    )
...
def test_ac5_null_check_for_max():
    ...
    assert has_null_check, (
        "training-log.js must guard zone2_hr_max for null before using it, "
        "falling back to ZONE2_HR_MAX when null/undefined"
    )
...
def test_ac6_prefs_used_via_null_safe_access():
    ...
    # Must safely guard the prefs object (could be null if fetch failed)
    has_safe_access = (
        "prefs && prefs.row" in rv_body
        or "prefs?.row" in rv_body
        or "(prefs || {})" in rv_body
        or "prefs ? prefs.row" in rv_body
    )
    assert has_safe_access, (
        "renderRunView must null-safely access the prefs argument (e.g. prefs && prefs.row) "
