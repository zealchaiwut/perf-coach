"""Tester verification for issue #280: server-side user identity for workouts,
training-log, trends, calendar, readiness, exports, and integration endpoints.

Verifies:
- Anonymous requests → 401 for all migrated endpoint groups
- Cross-user isolation: User A cannot read or mutate User B's workout data
- Session user is authoritative (body/query user_id ignored)
- Integration endpoints (Strava, Stryd, Google) require auth
"""
import uuid
...
class TestSameUserWorkoutAccess:
    ...
    def test_workout_post_uses_session_user(self, client, user_a, user_b, cookie_a):
        """POST with user_b in body must save under session user (user_a)."""
        cookies = {"session": cookie_a}
        res = client.post("/api/workouts", json={
...
class TestCrossUserIsolation:
    ...
    def test_exports_workouts_does_not_expose_user_b(self, client, user_b, cookie_a, workout_b):
        res = client.get("/api/exports/workouts", cookies={"session": cookie_a})
        assert res.status_code == 200
        assert user_b["id"] not in res.text, "User A's export must not contain User B's data"
