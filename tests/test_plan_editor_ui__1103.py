...
def user_and_client():
    ...
    r = client.post("/api/plans", json={"name": "Test Plan 1103"})
...
def test_add_race_form_has_type_field(user_and_client):
    """AC1/AC2: Plan modal has a type field showing race/checkpoint options via segmented control."""
    client, _ = user_and_client
    r = client.get("/log")
    html = r.text
    assert 'id="plan-modal-typeseg"' in html, "Type segmented control (plan-modal-typeseg) not found in plan modal"
    assert 'data-type="race"' in html, "Race option not found in type segmented control"
    assert 'data-type="checkpoint"' in html, "Checkpoint option not found in type segmented control"
...
def test_create_checkpoint_via_plan_router(user_and_client):
    ...
    client.delete(f"/api/plans/{plan_id}/races/{cp_id}")
...
def test_list_returns_races_and_checkpoints(user_and_client):
    ...
    client, plan_id = user_and_client
    r1 = client.post(f"/api/plans/{plan_id}/races", json={
        "name": "A Race",
        "date": "2027-09-01",
        "distance": 21.1,
        "type": "race",
    })
    r2 = client.post(f"/api/plans/{plan_id}/races", json={
        "name": "A Checkpoint",
        "date": "2027-07-01",
        "distance": 10.0,
        "type": "checkpoint",
    ...
    resp = client.get(f"/api/plans/{plan_id}/races")
...
def test_patch_race_updates_name(user_and_client):
    ...
    patch = client.patch(f"/api/plans/{plan_id}/races/{race_id}", json={"name": "Updated Name"})
...
def test_patch_race_updates_goal_time(user_and_client):
    ...
    patch = client.patch(
        f"/api/plans/{plan_id}/races/{race_id}",
...
def test_delete_race_removes_from_list(user_and_client):
    ...
    d = client.delete(f"/api/plans/{plan_id}/races/{race_id}")
...
def test_race_persists_across_requests(user_and_client):
    ...
    r3 = client.get(f"/api/plans/{plan_id}/races/{race_id}")
...
def test_training_log_html_has_type_select():
    """AC1/AC2: training-log.html plan modal has a type segmented control with race/checkpoint options."""
    ...
    assert 'id="plan-modal-typeseg"' in source, "Type segmented control (plan-modal-typeseg) missing from modal"
    assert 'data-type="race"' in source, "'race' option missing from type segmented control"
    assert 'data-type="checkpoint"' in source, "'checkpoint' option missing from type segmented control"
