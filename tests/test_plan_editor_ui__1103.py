...
# ── JS wiring: training-plan.js uses plan router ─────────────────────────────

def test_training_plan_js_references_plan_router():
    """AC3/AC5: training-plan.js uses the plan router URL pattern (/plans/)."""
    js_path = _ROOT / "frontend" / "js" / "training-plan.js"
    assert js_path.exists(), f"training-plan.js not found at {js_path}"
    source = js_path.read_text()
    assert "/plans/" in source, (
        "training-plan.js must reference the plan router (/plans/) — "
        "found only old /api/races references"
    )


def test_training_plan_js_uses_patch_for_edits():
    """AC5: training-plan.js uses PATCH method for editing races."""
    js_path = _ROOT / "frontend" / "js" / "training-plan.js"
    source = js_path.read_text()
    assert "PATCH" in source or "apiPatch" in source, (
        "training-plan.js must use PATCH for edits to match the plan router"
    )


def test_training_plan_js_validates_name():
    """AC9: training-plan.js validates that name is not empty before submitting."""
    js_path = _ROOT / "frontend" / "js" / "training-plan.js"
    source = js_path.read_text()
    assert "Name is required" in source or "name" in source.lower(), (
        "training-plan.js should validate the name field"
    )
