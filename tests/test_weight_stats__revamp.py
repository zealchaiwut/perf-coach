...
# ── Issue #1697: window-appropriate min_entries ───────────────────────────────


def test_7day_window_with_5_entries_is_readable():
    """AC: 7-day window with 5 entries (71% coverage) is readable after fix.

    Before the fix, weight_stats() always passed min_entries=21 regardless of
    window_days. A 7-day window can never contain 21 entries so readable was
    permanently False, blocking consecutive_weeks_behind in cut_review.py.
    """
    session = _new_session()
    user = _make_user(session)
    for i in range(5):
...
def test_7day_window_with_2_entries_is_not_readable():
    """AC: 2 entries in a 7-day window is below the proportional floor (5)."""
    session = _new_session()
    user = _make_user(session)
...
def test_30day_window_still_requires_21_entries():
    """Regression: 30-day window keeps the 21-entry gate unchanged."""
    session = _new_session()
    user = _make_user(session)
...
def test_days_needed_reflects_short_window_minimum():
    """AC: days_needed counts against the proportional window minimum, not global 21."""
    session = _new_session()
    user = _make_user(session)
    # 3 entries in a 7-day window; proportional min = 5, so gap = 2
    for i in range(3):
        _add_entry(session, user.id, AS_OF - datetime.timedelta(days=i * 2), 80.0 - i * 0.05)
    stats = weight_stats(session, user.id, window_days=7, as_of=AS_OF)
    assert stats["readable"] is False
    # days_needed must be based on ~5 (short-window min), not 21
...
def test_consecutive_weeks_behind_reaches_recalibrate_maintenance(_cut_today, _stats_today):
    """AC: recalibrate_maintenance is reachable via the real DB path after the fix.
    ...
    With window-appropriate min_entries, 3 consecutive behind weeks resolve and
    the recommendation fires.
    """
    session = _new_session(with_fuel=True)
    user = _make_user(session)

    # 21 daily entries; actual rate ~ -0.21 kg/wk, plan = -0.35 kg/wk → behind.
    # Rate is slow enough not to hit the guardrail penalty zone (< 0.625 %/wk)
    # and fast enough not to trigger plateau (< -0.1 kg/wk).
    for i in range(21):
