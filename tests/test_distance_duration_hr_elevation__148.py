"""
Acceptance tests for issue #148: Add distance, duration, heart rate, and elevation to workouts
...
def _create_workout(client, user_id, **kwargs):
    payload = {
        "user_id": user_id,
        "name": kwargs.pop("name", "Issue 148 Test Workout"),
...
# ── UAT Step 4: POST with new fields returns 201 with correct values ──────────
...
# ── UAT Step 5: GET returns the same persisted values ────────────────────────
...
# ── UAT Step 6: PATCH updates max_hr and elevation_m without touching others ──
...
def test_patch_updates_max_hr_and_elevation(client, alice_id):
    ...
    try:
        res = client.patch(f"/api/workouts/{w['id']}", json={"max_hr": 178, "elevation_m": 120})
        assert res.status_code == 200
        body = res.json()
        assert body["max_hr"] == 178
        assert body["elevation_m"] == 120
        assert body["distance_km"] == pytest.approx(8.0)
        assert body["duration_seconds"] == 2322
        assert body["avg_hr"] == 162
    finally:
...
# ── UAT Step 7: strength workout without new fields → all five null ───────────

def test_post_strength_workout_without_new_fields_has_null_metrics(client, alice_id):
    payload = {
        "user_id": alice_id,
        "workout_date": "2026-05-29",
...
# ── UAT Step 3: pre-existing workout returns five new fields as null ──────────

def test_workout_without_new_fields_returns_nulls(client, alice_id):
    w = _create_workout(client, alice_id, workout_type="strength", name="Pre-migration Workout")
    try:
        res = client.get(f"/api/workouts/{w['id']}")
        assert res.status_code == 200
        body = res.json()
...
# ── UAT Step 8: avg_hr: 19 returns 422 ───────────────────────────────────────

def test_post_avg_hr_19_returns_422(client, alice_id):
    payload = {
        "user_id": alice_id,
        "workout_date": "2026-05-29",
        "workout_type": "run",
...
def test_post_max_hr_251_returns_422(client, alice_id):
    payload = {
        "user_id": alice_id,
        "workout_date": "2026-05-29",
        "workout_type": "run",
...
# ── UAT Step 9: distance_km: -1 returns 422 ──────────────────────────────────
