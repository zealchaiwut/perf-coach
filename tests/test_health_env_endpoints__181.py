"""
Tests for issue #181: health check and environment metadata endpoints.
...
def test_home_page_includes_env_js(client):
    res = client.get("/home.html")
    assert res.status_code == 200
    assert "env.js" in res.text


def test_home_page_includes_env_label(client):
    res = client.get("/home.html")
    assert res.status_code == 200
    assert 'env-label' in res.text
