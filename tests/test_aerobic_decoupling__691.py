...
class TestNormalCasePowerAndHR:
    def test_returns_result_not_none(self):
        ...
        result, reason = compute_decoupling(WORKOUT_BASIC, stream, threshold=None)
    ...
    def test_uses_power_over_speed_when_both_present(self):
        ...
        result, _ = compute_decoupling(WORKOUT_BASIC, stream, threshold=None)
        # power-based: (250/140 - 240/140) / (250/140) * 100 = (10/140) / (250/140) * 100 = 10/250 * 100 = 4.0
...
class TestNormalCaseSpeedAndHR:
    ...
    def test_debug_first_efficiency_speed_based(self):
        ...
        expected = 3.0 / 140.0
        assert result["debug"]["first_half_efficiency"] == pytest.approx(expected, rel=0.01)
...
class TestMissingStreamInsufficientLaps:
    def test_none_stream_returns_null(self):
        result, reason = compute_decoupling(WORKOUT_BASIC, None, threshold=None)
    ...
    def test_single_split_returns_null(self):
        ...
        splits = [
            {"split_index": 0, "duration_seconds": 600, "avg_hr": 140, "avg_power": 250}
        ]
        result, reason = compute_decoupling(WORKOUT_BASIC, splits, threshold=None)
        assert result is None
        assert "insufficient" in reason.lower() or "lap" in reason.lower() or "split" in reason.lower()
...
class TestMissingHRData:
    ...
    def test_splits_no_hr_returns_null(self):
        ...
        splits = [
            {"split_index": 0, "duration_seconds": 300, "avg_hr": None, "avg_power": 250},
            {"split_index": 1, "duration_seconds": 300, "avg_hr": None, "avg_power": 240},
        ]
        result, reason = compute_decoupling(WORKOUT_BASIC, splits, threshold=None)
        assert result is None
        assert reason is not None
